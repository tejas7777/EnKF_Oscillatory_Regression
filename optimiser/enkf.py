...
class EnKFOptimizer:
    ...
    def calculate_jacobian_and_gradient(self, F, D):
        ...
        # Retrieve current parameter state in an appropriate structure
        current_params_unflattened = self.unflatten_parameters(self.theta)

        # Define a function that accepts a tuple of parameters, applies F and D to compute the loss
        ...
        # Ensure params are passed as a tuple to be treated as a single input
        ...
        # The output jacobian may still be a tuple depending on the output of D(output)
        # Extract the tensor from the tuple if necessary, and ensure it's the correct shape
